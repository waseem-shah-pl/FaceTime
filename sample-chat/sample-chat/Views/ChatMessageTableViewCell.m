...
- (void)configureCellWithMessage:(QBChatAbstractMessage *)message Width:(int)width
{
    NSLog(@"Width %d",width);
...
//        self.dateLabel.text = [NSString stringWithFormat:@"%@, %@", [[LocalStorageService shared].currentUser login], time];
        ...
        [self.messageTextView setFrame:CGRectMake(width-size.width-padding/2, padding+5, size.width, size.height+padding)];
        [self.messageTextView sizeToFit];
        
        [self.backgroundImageView setFrame:CGRectMake(width-size.width-padding/2, padding+5,
                                                      self.messageTextView.frame.size.width+padding/2, self.messageTextView.frame.size.height+5)];
